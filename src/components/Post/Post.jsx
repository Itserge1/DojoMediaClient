...
const Post = ({post}) => {
    ...
    // Get all users
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_LINK}/api/finduser/${post.userId}`)
            .then(res =>{
    ...
    const Cliked = (object) => {
        if(object=="like"){
            ...
            axios.put(`${process.env.REACT_APP_LINK}/api/post/like/${post._id}`, {_id: LoggedInUser._id})
                .then(res => {
                    console.log(res);
                })
                .catch(err => {
    ...
    // Delete post
    const deletePost = () => {
        axios.delete(`${process.env.REACT_APP_LINK}/api/delete/post/${post._id}`, {withCredentials:true})
            .then(res => {
